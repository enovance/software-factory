...
- Usage of lastest packages regarding securities breach.

In order to deploy an instance of Software Factory you need to use the images where all
components (Gerrit, JJB, ...) as well as base OS, kernel reside. For each Software Factory
release we provide those images, and we use those images approch to perform upgrades.
...
First note that we only support upgrade from latest TAG-1 to the latest TAG.
And we encourage to upgrade an already deployed SF platform at each new
version.
