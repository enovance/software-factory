...
class TestGerrit(Base):
    ...
    def test_gitweb_access(self):
        ...
        # Test if access to refs/meta/config is disabled for anonymous users
        # This requires a template change in managesf
        url = "%s/r/gitweb?p=%s.git;h=refs/meta/config" % (
            config.GATEWAY_URL, priv_pname)
        resp = requests.get(url)
        self.assertTrue("auth/login" in resp.url)
        self.assertFalse(expected_title in resp.text)
