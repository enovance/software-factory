...
function fetch_bootstraps_data {
    ...
    rsync -a -L ${SF_HOST}:/etc/software-factory/ sf-bootstrap-data/software-factory/
    ADMIN_PASSWORD=$(cat sf-bootstrap-data/software-factory/sfconfig.yaml | grep 'admin_password:' | sed 's/^.*admin_password://' | awk '{ print $1 }' | sed 's/ //g')
