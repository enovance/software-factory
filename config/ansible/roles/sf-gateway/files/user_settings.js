...
// Get user informations
    $http.get('/manage/services_users/?username='+localStorage.getItem("sf.username")).
        success(function(data) {
            ...
            if ($scope.user.idp_sync == "True") {
                $scope.user.idp_sync = true
            } else {
                $scope.user.idp_sync = false
            }
    ...
    $http.get('/manage/htpasswd/')
        .success( function (data) {
          ...
          $scope.user.gerrit_api_key = "SECRET";
        })
        .error( function (data) {
          $scope.htpasswd_set = false;
          $scope.user.gerrit_api_key = "-";
        })
        .catch( function (data) {
          $scope.htpasswd_set = false;
          $scope.user.gerrit_api_key = "-";
    ...
    $scope.htpasswd_enable = function(name) {
        ...
        $http.put('/manage/htpasswd/')
            .success(function(data) {
                $scope.user.gerrit_api_key = data;
                $scope.htpasswd_set = true;
            })
            .error(function(data) {
                $scope.errors = data;
