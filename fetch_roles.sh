...
function fetch_base_roles_prebuilt {
    ...
    [ ! -d cloud ] && {
        ...
        [ "$(md5sum $cloud_img | awk '{ print $1 }')" == "$(cat $cloud_img.md5)" ] || die "Image md5 checksum failed"
    ...
    [ ! -d install-server ] && {
        echo "Check image checksum ..."
        [ "$(md5sum $install_server_img | awk '{ print $1 }')" == "$(cat $install_server_img.md5)" ] || die "Image md5 checksum failed"
