...
function run_provisioner {
    ...
    # The fqdn in sf-bootstrap-data/hiera/sfconfig.yaml might be the old one
    # before the upgrade. We can't change it right now, because the certificates
    # have not been changed yet; therefore it is required to have the old and
    # new fqdn in /etc/hosts
    CURRENT_FQDN=`grep fqdn sf-bootstrap-data/hiera/sfconfig.yaml | cut -c 7-`
    echo "192.168.135.101 ${CURRENT_FQDN}" | sudo tee -a /etc/hosts
    . /var/lib/sf/venv/bin/activate
