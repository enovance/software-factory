...
def set_node_options(item, job, params):
    print "XXXDEBUG: set_node_options(", item, job, params, ")"
    build = item.current_build_set.getBuild(job.name)
    print "XXXDEBUG: build(", job.name, ") is", build
    if build:
    ...
    if offline_node:
        print "XXXDEBUG: Setting OFFLINE_NODE_WHEN_COMPLETE for job %s" % job.name
