...
function get_logs {
    ...
    for i in gerrit  jenkins  managesf  mysql  puppetmaster  slave; do
        mkdir -p ${O}/$i/system
...
function run_puppet_allinone_tests {
    ...
    ssh -o StrictHostKeyChecking=no root@`get_ip puppetmaster` \
            "puppet master --compile allinone --environment=sf" 2>&1 | tee ${ARTIFACTS_DIR}/functional-tests.output
    return ${PIPESTATUS[0]}
}
