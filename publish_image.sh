...
function publish {
    ...
    UPSTREAM_HASH=$(curl -s ${SWIFT_SF_URL}/${IMG_NAME}.hash &> /dev/null)
    # Swift does not return 404 but 'Not Found'
    [ "$(cat ${IMG_NAME}.hash)" == "${UPSTREAM_HASH}" ] && return
    ...
    md5sum $OBJ | sudo tee ${IMG_NAME}.md5sum
    for OBJECT in $OBJ ${IMG_NAME}.md5sum; do
        [ -f ${OBJECT} ] || continue
