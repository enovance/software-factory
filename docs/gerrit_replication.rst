...
In the same spirit of the job' definitions, the replication should
be configured through the config repository. Under the directory
...
Here we configure the replication plugin of Gerrit to replicate project
where their names match the regular expression "^myproject-.*" to the remote
server mirrors.domain.example. The special placeholder "${name}" is needed here
because the regular explression will match several projects. Furthermore please
note the "mirror" option is used in order to replicate branch deletion.
...
The patch will be reviewed and approved by the folk currenlty CORE or PTL
...
Recommandation for the replication of GitHub
--------------------------------------------
...
.. code-block:: bash
 ...
 $ cat deploy-key.pub | ssh root@sftests.com gerrit_repl_alias_helper \
...
.. code-block:: bash
 ...
 $ gerrit_repl_alias_helper --hostname github.com --key-path \
