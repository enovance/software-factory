...
function run_functional_tests {
    ...
    ssh -o StrictHostKeyChecking=no root@`get_ip puppetmaster` bash -c "for i in puppet-templates/*.pp; do echo ==\$i==; cat \$i; puppet apply --environment sf --debug --noop --test \$i; done" 2>&1 > ${ARTIFACTS_DIR}/puppet-apply-tests.output
