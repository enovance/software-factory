...
function clean_nodepool_tenant {
    ...
    for image in $(nova image-list | grep template | awk '{ print $2 }'); do nova image-delete $image; done
...
function run_it_openstack {
    ...
    ${it_cmd} zuul      || echo "Basic integration test failed"
    ${it_cmd} nodepool --os_base_image "sf-${SF_VER}" --os_user sfnodepool || echo "Nodepool integration test failed"
    ${it_cmd} swiftlogs || echo "Swift integration test failed"
