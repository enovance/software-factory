...
export SF_HOST=${SF_HOST:-tests.com}
export SKIP_CLEAN_ROLES="y"

MANAGESF_URL=https://${SF_HOST}
ADMIN_USER=admin
ADMIN_PASSWORD=userpass
JENKINS_URL="http://${SF_HOST:-tests.com}/jenkinslogs/${JENKINS_IP}:8081/"
...
function run_backup_restore {
    ...
    while [ $retry -lt 1000 ]; do
        wget --spider  http://tests.com/r/ 2> /dev/null && break
        sleep 1
        let retry=retry+1
    done
