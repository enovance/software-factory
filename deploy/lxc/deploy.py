...
def port_redirection(mode):
    ...
    for port in (80, 443, 8080, 29418, 45452):
...
def init(base):
    ...
    if args.refarch == "2nodes-jenkins":
        ...
        for node, prefix in distributed_nodes:
            prepare_role(base, node,  "192.168.135.%d" % prefix)
...
def start():
    ...
    if args.refarch == "2nodes-jenkins":
        ...
        for node, prefix in distributed_nodes:
            open("libvirt-%s.xml" % node, "w").write(
                open("libvirt-jenkins.xml").read().replace(
                    "jenkins", node).replace(
                    # Replace ip
                    "102", str(prefix)).replace(
                    # Replace mac addr
                    "56", str(prefix - 100)))
            virsh(["create", "libvirt-%s.xml" % node])
    port_redirection("up")
    virsh(["list"])
