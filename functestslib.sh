...
function fail {
    set +x
    DISABLE_SETX=1
    echo "\n$(hostname) FAIL to run functionnal tests of this change: $1"
    display_head
...
function run_bootstraps {
    ...
    ssh -A -tt ${SF_HOST} "cd bootstraps; exec ./bootstrap.sh ${REFARCH}" 2>&1 | tee ${ARTIFACTS_DIR}/bootstraps.log | grep '\(Info:\|Warning:\|Error:\)'
    res=${PIPESTATUS[0]}
    kill -9 $SSH_AGENT_PID
    if [ "$res" != "0" ]; then
        tail -n 500 ${ARTIFACTS_DIR}/bootstraps.log
...
 
venv_python=/var/lib/sf/venv/bin/python
venv_nose=/var/lib/sf/venv/bin/nosetests
...
function run_provisioner {
    ...
    ${venv_python} ./tests/functional/provisioner/provisioner.py 2>> ${ARTIFACTS_DIR}/provisioner.debug || fail "Provisioner checker failed"
    ${venv_python} /var/lib/sf/venv/bin/sfmanager --url "${MANAGESF_URL}" --auth user1:userpass system backup_start || fail "Backup failed"
    ${venv_python} /var/lib/sf/venv/bin/sfmanager --url "${MANAGESF_URL}" --auth user1:userpass system backup_get || fail "Backup get failed"
}

function run_checker {
    echo "$(date) ======= run_checker"
    ${venv_python} /var/lib/sf/venv/bin/sfmanager --url "${MANAGESF_URL}" --auth user1:userpass system restore --filename $(pwd)/sf_backup.tar.gz || fail "Backup resore failed"
    ${venv_python} ./tests/functional/provisioner/checker.py 2>> ${ARTIFACTS_DIR}/checker.debug || fail "Backup checker failed"
}

function run_functional_tests {
    echo "$(date) ======= run_functional_tests"
    . /var/lib/sf/venv/bin/activate
    nosetests --with-xunit -v tests/functional > ${ARTIFACTS_DIR}/functional-tests.debug
    RES=$?
    deactivate
...
function run_puppet_allinone_tests {
    echo "$(date) ======= run_puppet_allinone_tests"
