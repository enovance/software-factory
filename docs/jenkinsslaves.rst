...
is still some manual commands to apply to let nodepool use the new configuration.
...
By default SF provide a build script called "base.sh" that is the minimal script to run
...
Jenkins slaves. The following is the process to define an image for Nodepool. Clone
the config repository of SF from Gerrit and modify the file "config/nodepool/images.yaml"
as below.
...
validation and will allow you to merge the change (by CR +2 and W +2). Once merged
the new configuration of nodepool will be load by the nodepool service. And you should
 ...
 * At least one VM is spawn based on the snapshot
