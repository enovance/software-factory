...
class TestResourcesWorkflow(Base):
    ...
    def test_validate_wrong_resource_workflow(self):
        """ Check wrong resource is detected by config-check """
...
"""
    ...
    def test_validate_correct_resource_workflow(self):
        """ Check good resource is detected by config-check """
...
"""
        # Add the resource file with review then check CI
        resources = resources % (name, name, namespace)
        self.propose_resources_change_check_ci(fpath,
                                               resources=resources,
                                               mode='add',
                                               expected='success')
    ...
    def test_CUD_group(self):
        """ Check operations on group work as expected """
        fpath = "resources/%s.yaml" % create_random_str()
        namespace = create_random_str()
        name = create_random_str()
        resources = """resources:
...
"""
        ...
        gid = self.gu.get_group_id(os.path.join(namespace, name))
...
"""
        ...
        self.assertFalse(
            self.gu.get_group_id(os.path.join(namespace, name)))

    def test_CD_repo(self):
        """ Check operations on git repositories work as expected """
        fpath = "resources/%s.yaml" % create_random_str()
        namespace = create_random_str()
        name = create_random_str()
        resources = """resources:
...
"""
    ...
    def test_CRUD_resources(self):
        """ Check bulk operations on resources work as expected """
        fpath = "resources/%s.yaml" % create_random_str()
