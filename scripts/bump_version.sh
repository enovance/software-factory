...
#
#  2/ Commit using 'TaggedRelease: 2.1.6'
#  3/ Run this script to update references
#  4/ Check subproject history + sf history to update changelog
#  5/ Commit --amend with all changes and new symlinks
#  6/ Wait for change to be merged (ask for review, recheck gate)
#  7/ Test new releases (either deploy or upgrade), in particular the changelog
#  8/ If published build is good, sign and re-publish the digest
...
#  9/ Generate changelog: reno report .
...
# 12/ Tag: git tag -a -m "${VER}" ${VER} HEAD
...
echo git add image/metadata/*/softwarefactory/*
...
for i in pysflib managesf python-sfmanager cauth; do
    (cd /var/lib/sf/deps/$i; git fetch origin &> /dev/null)
    PINNED_VERSION=$(cat /var/lib/sf/deps/$i/.git/FETCH_HEAD | awk '{ print $1 }')
    VNAME="$(echo $i | tr '[:lower:]' '[:upper:]')_PINNED_VERSION"
    echo sed -i role_configrc -e "s/${VNAME}=.*/${NAME}=${PINNED_VERSION}/"
done
