...
def merge(_nodepool):
    ...
    for diskimage in user['diskimages']:
        diskimage['env-vars']['TMPDIR'] = '/var/cache/nodepool/dib_tmp'
        diskimage['env-vars']['DIB_IMAGE_CACHE'] = '/var/cache/nodepool/dib_cache'
...
if __name__ == '__main__':
    ...
    for reqfile in (_nodepool, "nodepool/nodepool.yaml"):
        if not os.path.isfile(_nodepool):
            print "%s: missing file" % _nodepool
            sys.exit(1)
