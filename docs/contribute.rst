...
* Reporting problems found in the documentation could be a good way to
  help and contribute at the begining.

To report those problems feel free to contact us on Freenode or even create
a bug report on the bugs tracker: http://softwarefactory.enovance.com/_redmine/projects/software-factory
...
sofwarefactory source code. Note that we also use that project to build our
...
You can then jump to :ref:`lxcdeploy` to learn
...
Note: if the build role process take too long you can take a look at build log:

.. code-block:: bash

 $ tail -f /var/lib/sf/artifacts/logs/build_roles.sh.output
...
* Check the bugs tracker and the pending reviews
* Submit your change
