...
function register_images {
    for img in install-server-vm softwarefactory; do
        checksum=`glance image-show $STACKNAME_$img | grep checksum | awk '{print $4}'`
        if [ -z "$checksum" ]; then
            glance image-create --name $STACKNAME_$img --disk-format qcow2 --container-format bare \
...
function unregister_images {
    for img in install-server-vm softwarefactory; do
        ...
        [ "$newchecksum" != "$checksum" ] && glance image-delete $STACKNAME_$img || true
    done
}
