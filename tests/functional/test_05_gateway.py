...
class TestGateway(unittest.TestCase):
    ...
    def test_css_js_for_topmenu_accessible(self):
        ...
        url = "http://%s/static/bootstrap/js/bootstrap.min.js" % config.GATEWAY_HOST
        resp = requests.get(url)
        self.assertEqual(200, resp.status_code)
        self.assertTrue("Bootstrap v3.2.0" in resp.content)

        url = "http://%s/static/bootstrap/css/bootstrap.min.css" % config.GATEWAY_HOST
        resp = requests.get(url)
        self.assertEqual(200, resp.status_code)
        self.assertTrue("Bootstrap v3.2.0" in resp.content)
    ...
    def test_static_dir_for_paste_accessible(self):
        ...
        """
        url = "http://%s/lodgeit/static/jquery.js" % config.GATEWAY_HOST
        resp = requests.get(url)
        self.assertEqual(200, resp.status_code)
