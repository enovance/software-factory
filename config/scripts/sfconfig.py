...
def update_sfconfig(data):
    ...
    if not os.path.isfile("/etc/software-factory/topmenu-logo.png"):
        open("/etc/software-factory/topmenu-logo.png", "w").write(
            base64.decodestring(data['theme']['topmenu_logo_data']))
        del data['theme']['topmenu_logo_data']

    if not os.path.isfile("/etc/software-factory/favicon.ico"):
        open("/etc/software-factory/favicon.ico", "w").write(
            base64.decodestring(data['theme']['favicon_data']))
        del data['theme']['favicon_data']

    if not os.path.isfile("/etc/software-factory/splash.png"):
        # Import the default one
        open("/etc/software-factory/splash.png", "w").write(
            open("/usr/local/share/sf-default-config/splash.png").read())
...
""" % (sfconfig["fqdn"], sfconfig["fqdn"]))
    ...
    if "gateway" in arch["roles"]:
        ...
        glue["gateway_topmenu_logo_data"] = encode_image(
            "/etc/software-factory/topmenu-logo.png")
        glue["gateway_favicon_data"] = encode_image(
            "/etc/software-factory/favicon.ico")
        glue["gateway_splash_image_data"] = encode_image(
            "/etc/software-factory/splash.png")
