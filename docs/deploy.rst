...
Note that SF will use "tests.localdomain" as default FQDN and if the FQDN doesn't resolve it needs to be locally
...
with "tests.localdomain" as the FQDN and "admin/userpass" as admin credentials.
...
using "Internal Login". If you used the default domain *tests.localdomain* then
