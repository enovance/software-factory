...
function pre_fail {
    ...
    case $1 in
        "Roles building FAILED")
            if [ -f "${ARTIFACTS_DIR}/software-factory_build.log" ]; then
                F="${ARTIFACTS_DIR}/software-factory_build.log"
            elif [ -f "${ARTIFACTS_DIR}/install-server-vm_build.log" ]; then
                F="${ARTIFACTS_DIR}/install-server-vm_build.log"
            fi
            if [ -f "${F}" ]; then
                echo ${F}
                tail -n 120 ${F}
