...
function run_it_jenkins_ci {
    ./tests/integration/run.py --password ${ADMIN_PASSWORD} || fail "Basic integration test failed"
    checkpoint "run_it_jenkins_ci"
}

function run_it_openstack {
    # Export SF_VER so that nodepool can reuse sf glance image named sf-${SF_VER}
    export SF_VER
    it_cmd="./tests/integration/run.py --password ${ADMIN_PASSWORD} --playbook"
    ${it_cmd} zuul      || fail "Basic integration test failed"
    ${it_cmd} nodepool --os_user sfnodepool || fail "Nodepool integration test failed"
    ${it_cmd} swiftlogs || fail "Swift integration test failed"
    ${it_cmd} alltogether || echo "Alltogether integration test failed"
