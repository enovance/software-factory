...
function run_health_openstack {
    ...
    EXTRA_VARS="node=base_centos os_base_image=sf-${SF_VER} os_slave_network=${HEAT_SLAVE_NETWORK}"
    EXTRA_VARS+=" os_auth_url=${OS_AUTH_URL} os_username=${OS_USERNAME} os_password=${OS_PASSWORD} os_tenant_name=${OS_TENANT_NAME}"
    ssh ${SF_HOSTS} ansible-playbook "--extra-vars=${EXTRA_VARS}" /etc/ansible/health-check/nodepool.yaml >> ${ARTIFACTS_DIR}/integration_tests.txt \
        && echo "(non-voting) Nodepool integration test SUCCESS"    \
        || echo "(non-voting) Nodepool integration test failed" ${ARTIFACTS_DIR}/integration_tests.txt
