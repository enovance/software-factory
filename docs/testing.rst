...
a custom script that you may have in your project to retrieve all produced
logs/artifacts by a job/test. This custom publisher is executed whatever
the "./run_functional_tests.sh"'s result. All logs/artifacts must be copied in the
...
look for a local "artifacts" directory. This publisher assumes that
...
To ease the access to the logs/artifacts from the a Jenkins's console you
can display the url contained in the environment variable : "SWIFT_artifacts_URL".

Furthermore in order to trigger the creation, by Zuul, of environment variables
needed by zuul-swift-upload in you have to configure the job as follow
in (zuul/projects.yaml):
...
The publisher uses the formpost capability of Swift. You can also activate the
staticweb middleware to ease logs/artifacts browsing.

The Software Factory administrator should have configured Zuul via sfconfig.yaml
in order to allow you to use that feature.
