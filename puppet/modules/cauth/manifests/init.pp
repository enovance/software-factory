...
class cauth ($cauth = hiera('cauth'), $gerrit = hiera('gerrit')) {
  ...
  $hook_user_password = hiera('creds_sf_hooks_user_pwd')
  ...
  $hook_user_password_hashed = generate("/usr/bin/python", "-c", "import crypt, random, string, sys; salt = '\$6\$' + ''.join(random.choice(string.letters + string.digits) for _ in range(16)) + '\$'; sys.stdout.write(crypt.crypt(sys.argv[1], salt))", $hook_user_password)
