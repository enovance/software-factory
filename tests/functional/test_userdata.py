...
from utils import skipIfIssueTrackerMissing, is_present, has_issue_tracker
...
class TestUserdata(Base):
    ...
    def verify_userdata_issue_tracker(self, login):
        ...
        self.assertEqual(config.USERS[login]['lastname'], user[2])
    ...
    def test_delete_in_backend_and_recreate(self):
        ...
        if has_issue_tracker():
            users = self.rm.active_users()
            user = [u for u in users if u[0] == 'freddie'][0]
            tracker_id = self.rm.get_user_id(user)
        del_url = config.GATEWAY_URL +\
        ...
        if has_issue_tracker():
            users = self.rm.active_users()
            user = [u for u in users if u[0] == 'freddie'][0]
            new_tracker_id = self.rm.get_user_id(user)
