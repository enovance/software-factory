...
executed is not mandatory and you can define your owns.
...
The other files may be modified by an update of Software Factory and *shouldn't be modified manualy*.
...
by JJB and Zuul. Then the patch must be peer reviewed, accepted and pushished to master via
...
As mentionned above you may want to define custom jobs. To do that you need to checkout the config
...
As explained in the previous chapter you then need to submit and publish the change on Gerrit in order
your new job is handled.
