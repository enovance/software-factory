...
class TestManageSF(Base):
    ...
    def test_list_active_members(self):
        ...
        active_users = eval(self.msu.list_active_members(config.USER_2))
    ...
    def test_register_user(self):
        # TODO Dangerous ! but will have to do for now
        active_users = eval(self.msu.list_active_members(config.ADMIN_USER))
        self.msu.register_user(config.ADMIN_USER, "a", "b")
        new_a_u = eval(self.msu.list_active_members(config.ADMIN_USER))
