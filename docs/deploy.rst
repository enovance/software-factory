...
a new VM using the SF image and then login via the console using root user with an empty password.
...
If you intend to reconfigure the domain on an already deployed SF, please use
the *-d* option of *sfconfig.sh* script.
...
Furthermore SSH authentication via password is disabled. root user won't be able
to log via SSH using a password.

In environments like OpenStack a metadata server is accessible and the user public
key will be installed for root and centos users.

**Outside Openstack, when using a local hypervisor** at first root login via the
console the user need to add its public ssh key in */root/.ssh/authorized_key*.
