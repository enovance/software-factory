    ...
    $scope.membershipForm = function(projectName, projectObj) {
        ...
        var memberGroups = function ( member_id ) {
            ...
            for ( var j = 0; j < numGroups; j++ ) {
                var tmp = [groups[j]["name"], false];
                for ( var k = 0; k < groups[j]["members"].length; k ++ ) {
                    if ( groups[j]["members"][k].match( member_id ) )
                        tmp[1] = true;
                }
        ...
        for ( i = 0; i < numMembers; i++ ) {
            ...
            $scope.originalMembers[$scope.members[i]["id"]] = {name: member["name"],
                                                               groups: memberGroups(member["id"])};
        }
        ...
        for ( var key in $scope.originalMembers ) {
            if ( $scope.originalMembers.hasOwnProperty(key) ) {
                ...
                for( x = 0; x < groups.length; x++) {
                    if ( groups[x][1] ) {
                        console.log("Add Member ", $scope.originalMembers[key].name, groups[x][0]);
                    }
                    else {
                        console.log("Remove Member ", $scope.originalMembers[key].name, groups[x][0]);
                    }
                }
