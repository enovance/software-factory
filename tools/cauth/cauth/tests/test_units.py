...
class TestCauthApp(FunctionalTest):
    ...
    def test_get_logout(self):
        # Set a SSO cookie first and ensure it is deleted after logout
        self.app.set_cookie('auth_pubtkt', 'something')
        response = self.app.get('/logout')
        self.assertEqual(response.status_int, 200)
        self.assertFalse('auth_pubtkt=' in response.headers['Set-Cookie'])
