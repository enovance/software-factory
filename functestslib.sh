...
export SF_HOST=${SF_HOST:-tests.localdomain}
export SKIP_CLEAN_ROLES="y"

MANAGESF_URL=https://${SF_HOST}
ADMIN_USER=admin
ADMIN_PASSWORD=userpass
JENKINS_URL="http://${SF_HOST:-tests.localdomain}/jenkinslogs/${JENKINS_IP}:8081/"
...
function configure_network {
    if [ "${SF_HOST}" != "tests.localdomain" ]; then
...
function run_backup_restore {
    ...
    while [ $retry -lt 1000 ]; do
        wget --spider  http://tests.localdomain/r/ 2> /dev/null && break
        sleep 1
        let retry=retry+1
    done
