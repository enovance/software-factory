...
function build_image {
    ...
    if [ -z "${SKIP_BUILD}" ]; then
        ...
        set -e
        sudo rsync -aiv --delete puppet/manifests/ ${IMAGE_PATH}/etc/puppet/environments/sf/manifests/
        sudo rsync -aiv --delete puppet/modules/ ${IMAGE_PATH}/etc/puppet/environments/sf/modules/
        sudo rsync -aiv --delete puppet/hiera/ ${IMAGE_PATH}/etc/puppet/hiera/sf/
        sudo rsync -aiv --delete config/ansible/ ${IMAGE_PATH}/usr/local/share/sf-ansible/
        sudo rsync -aiv --delete config/config-repo/ ${IMAGE_PATH}/usr/local/share/sf-config-repo/
        sudo rsync -aiv config/scripts/ ${IMAGE_PATH}/usr/local/bin/
        sudo rsync -aiv config/defaults/ ${IMAGE_PATH}/etc/puppet/hiera/sf/
