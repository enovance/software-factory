...
function build_image {
    # Image hash is last commit of DEPS and the one that changed content of the image (bootstraps/ docs/ image/ puppet/)
    IMAGE_HASH="SF: $(git log --format=oneline -n 1 config/ docs/ image/ puppet/) || CAUTH: $(cd ${CAUTH_CLONED_PATH}; git log --format=oneline -n 1) || PYSFLIB: $(cd ${PYSFLIB_CLONED_PATH}; git log --format=oneline -n 1) || MANAGESF: $(cd ${MANAGESF_CLONED_PATH}; git log --format=oneline -n 1)"
