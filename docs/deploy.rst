...
The default architecture (all in one instance) requires about minimum 40GB of hard drive and 4GB of memory with swap.
80GB of hard drive and 8GB of memory enough for a normal deployment (about 20 users).
...
* Edit the configuration sfconfig.yaml
** Set the fqdn required by OAuth authentication callback
** Set the local admin username and admin password
* Run configuration script
* If settings are changed after deployment, re-run configuration script (bootstraps.sh)

.. code-block:: bash

 $ ssh -A root@main_instance
 [root@managesf ~]# cd bootstraps
 [root@managesf bootstraps]# vim sfconfig.yaml
 [root@managesf bootstraps]# ./bootstraps.sh

After this process is completed, SF is ready to used.
...
Then open your browser on http://FQDN (TCP/80 must be allowed
