...
for i in image/metadata/C7.0-2.0*/softwarefactory/C7.0-*; do echo mv $i $(dirname $i)/${SF_VER}; done
echo "# Update prev version to new path"
for i in image/metadata/C7.0-2.1*/softwarefactory/C7.0-*; do echo mv $i $(dirname $i)/${NEW_VER}; done
...
for i in upgrade/C7.0-2.0*/*; do echo mv $i $(dirname $i)/${SF_VER}; done
echo "# Update prev verions"
for i in upgrade/C7.0-2.1*/*; do echo mv $i $(dirname $i)/${NEW_VER}; done
echo "# Create new path"
