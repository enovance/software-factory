...
function run_it_jenkins_ci {
    ...
    ./tests/integration/run.py > ${ARTIFACTS}/integration_tests.txt \
        || fail "Basic integration test failed" ${ARTIFACTS}/integration_tests.txt
    checkpoint "run_it_jenkins_ci"
}

function run_it_openstack {
    it_cmd="./tests/integration/run.py --password ${ADMIN_PASSWORD} --playbook"
    ${it_cmd} zuul >> ${ARTIFACTS}/integration_tests.txt \
        || fail "Basic integration test failed" ${ARTIFACTS}/integration_tests.txt
    ${it_cmd} nodepool --os_base_image "sf-${SF_VER}" --os_user sfnodepool &>> ${ARTIFACTS}/integration_tests.txt \
        || echo "(non-voting) Nodepool integration test failed"
    ${it_cmd} swiftlogs >> ${ARTIFACTS}/integration_tests.txt \
        || echo "(non-voting) Swift integration test failed"
    ${it_cmd} alltogether >> ${ARTIFACTS}/integration_tests.txt \
...
function build_image {
    ...
    if [ -z "${SKIP_BUILD}" ]; then
        echo "[+] Building image to ${IMAGE_PATH}"
        ./build_image.sh 2>&1 | tee ${ARTIFACTS_DIR}/image_builds.log | grep '(STEP'
        [ "${PIPESTATUS[0]}" == "0" ] || fail "Roles building FAILED" ${ARTIFACTS_DIR}/image_builds.log
...
function prepare_functional_tests_venv {
    ...
    (
        ...
        pip install --upgrade 'setuptools' 'pbr' 'pycrypto' 'pycrypto==2.6.1' 'urllib3==1.11' 'python-redmine==1.2.0' 'pyyaml==3.11' 'six==1.10.0' '-e git+https://github.com/morucci/pygerrit.git#egg=pygerrit'
