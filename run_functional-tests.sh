...
case "${TEST_TYPE}" in
    ...
    "upgrade")
        ...
        for ARCH_FILE in /etc/puppet/hiera/sf/_arch.yaml /etc/puppet/hiera/sf/arch.yaml; do
            ssh sftests.com "python -c \"import sys,yaml;f=sys.argv[1];comps=sys.argv[2:];cont=yaml.load(file(f));cont['inventory'][0]['roles'].extend(comps);yaml.dump(cont,sys.stdout,default_flow_style=False,indent=4);\" $ARCH_FILE $TECH_PREVIEW > $ARCH_FILE"
        done
        run_upgrade
