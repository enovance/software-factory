...
source $(dirname $(readlink $0 || echo $0))/scripts_lib.sh
lock

SF_ROLES=${SF_ROOT}/edeploy/
...
[ -z "$SKIP_UPSTREAM" ] && {
    [ -z "$SF_SKIP_FETCHBASES" ] && {
        ${SF_ROOT}/scripts/fetch_roles.sh bases || echo "pass..."
        echo
    }
    ${SF_ROOT}/scripts/fetch_roles.sh trees || echo "pass..."
    echo
    [ -n "$VIRT" ] && {
        ${SF_ROOT}/scripts/fetch_roles.sh imgs || echo "pass..."
    }
}
