...
case "${TEST_TYPE}" in
    ...
    "upgrade")
        ...
        for ARCH_FILE in /etc/puppet/hiera/sf/_arch.yaml /etc/puppet/hiera/sf/arch.yaml; do
            ...
            ssh sftests.com "python -c \"import sys,yaml;f=sys.argv[1];comps=sys.argv[2:];cont=yaml.load(file(f));cont['inventory'][0]['roles'].extend(comps);yaml.dump(cont,file(f,'w'),default_flow_style=False,indent=2);\" $ARCH_FILE $TECH_PREVIEW"
        done
        run_upgrade
