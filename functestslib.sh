...
function run_backup_restore_tests {
    ...
    if [ "$type" == "provision" ]; then
        ...
        # Create a backup
        ssh -o StrictHostKeyChecking=no root@`get_ip puppetmaster` "cd puppet-bootstrapper/tools/managesf/cli; python sf-manage.py --url ${MANAGESF_URL} --auth-url ${MANAGESF_URL} --auth user1:userpass backup_start"
        sleep 10
        # Fetch the backup
        ssh -o StrictHostKeyChecking=no root@`get_ip puppetmaster` "cd puppet-bootstrapper/tools/managesf/cli; python sf-manage.py --url ${MANAGESF_URL} --auth-server ${MANAGESF_URL} --auth user1:userpass backup_get"
