...
function get_logs {
    ...
    ssh sftests.com ansible-playbook /etc/ansible/get_logs.yml &> ${ARTIFACTS_DIR}/get_logs.log && {
        scp -r sftests.com:sf-logs/* ${ARTIFACTS_DIR}/ > /dev/null
...
function pre_gui_tests {
    ...
    /usr/bin/java -jar /usr/lib/selenium/selenium-server.jar -host 127.0.0.1 &> ${SCREENSHOT_DIR}/selenium.log &
    echo "Starting Xvfb in background ..."
    Xvfb :99 -ac -screen 0 1920x1080x24 &> ${SCREENSHOT_DIR}/Xvfb.log &
    sleep 3
...
function post_gui_tests {
    ...
    if [ "${GUI_TEST_SUCCESS}" == "0" ]; then
        more ${ARTIFACTS_DIR}/selenium.log ${SCREENSHOT_DIR}/Xvfb.log | cat
...
function run_gui_test {
    ...
    export DISPLAY=:99
    [ -z "${SKIP_RECORDING}" ] || {
        tmux new-session -d -s guiTestRecording_${RANDOM} "export FFREPORT=file=${SCREENSHOT_DIR}/ffmpeg-$(date +%Y%m%s).log && exec ffmpeg -f x11grab -video_size 1920x1080 -i 127.0.0.1$DISPLAY -codec:v mpeg4 -r 16 -vtag xvid -q:v 8 ${SCREENSHOT_DIR}/$1.avi"
