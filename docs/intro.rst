...
Software Factory is a collection of services that provides
a powerful platform to produces software. It's designed to
run on OpenStack based cloud but it can also be used standalone
without the feature using the cloud like tests environments.


What is Software Factory
------------------------

Software Factory (also called SF) integrates services for each element of a software
production chain:
...
* Contrinous integration : `Zuul <http://ci.openstack.org/zuul/>`_
...
SF offer a seamless user experience with:

* Single sign on with cauth
* top-menu to access all services
* dashboard

SF tests jobs, instances and pipelines configuration is done through a special project
called config and is updated through SF workflow (using code review and zuul gate).
...
SF image can be uploaded to a Glance service, deployed on
...
SF do not requires internet access to be installed, and it comes with
an automated upgrade process.
...
* Plugins interface for other issue tracker like Jira or Phabricator
